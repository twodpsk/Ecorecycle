...
### 1. 학급 및 학교 단위 경쟁 시스템
- 학급별·학교별 분리수거 실천 점수 경쟁 시스템 개발함.  
- 기업·교육청·학교 연계 보상 시스템 기획함.  
- 점수·랭킹·배지 시스템 설계함.

### 2. 가정 연계 분리수거 인증
- 학생 개인 폰 연동을 통한 가정 내 분리수거 사진 업로드 기능 구현함.  
- 업로드 사진 기반 학습 포인트 산정 기능 개발함.  
- 인증 기록의 학습 진도 및 실천 성과 반영 기능 설계함.

### 3. 환경 센터 지도 연동
- Google Maps API 연동을 통한 환경 관련 센터(아름다운 가게 등) 위치 마커 표시 기능 구현함.  
- 센터 리스트 기능 및 방문/기부 시 포인트 연동 기능 설계함.

### 4. 카메라 기반 AI 분류 모델 구축
- 분리수거 촬영 이미지 기반 재질(플라스틱·캔·종이·유리·일반쓰레기) 자동 판별 모델 개발 계획함.  
- 학습 데이터 수집 및 라벨링 파이프라인 구축 계획함.  
- 모델 성능 모니터링 및 지속적 개선 프로세스 설계함.

### 5. 디자인 및 UX 개선
- 앱 UI/UX 디자인 수정 및 반응형 화면 설계 진행함.
