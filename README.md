...
****************************진행중/계획***********************************************
### 1. 학급 및 학교 단위 경쟁 시스템
- 학급별, 학교별 분리수거 실천 점수 경쟁 기능
- 기업·교육청·학교에서 학습 또는 실천 성과에 따라 보상 제공 가능
- 점수, 랭킹, 배지 시스템 구성

### 2. 가정 연계 분리수거 인증
- 학생 개인 폰과 연동하여 **집에서 분리수거한 사진 업로드**
- 업로드된 사진을 기반으로 학습 포인트 생성
- 인증 기록은 학습 진도와 실천 성과에 반영

### 3. 환경 센터 지도 연동
- Google Maps API 기반  
- 아름다운 가게, 환경센터, 재활용 센터 등 위치 마커 표시
- 리스트 기능 제공 + 센터 방문 시 포인트 또는 기록 반영

### 4. 카메라 기반 AI 분류 모델 구축
- 분리수거 촬영 이미지로 재질 자동 분류 모델 개발
- 사진 업로드 시 AI가 ‘플라스틱/캔/종이/유리/일반쓰레기’ 등 판별
- 학습 데이터 확보 후 지속적으로 모델 성능 개선
